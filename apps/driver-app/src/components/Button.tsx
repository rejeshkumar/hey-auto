...
export function Button({
  title, onPress, variant = 'primary', size = 'lg', loading = false,
  disabled = false, icon, style, textStyle, fullWidth = true,
}: ButtonProps) {
  ...
  return (
    ...
    >
      {loading ? (
        <ActivityIndicator color={variant === 'primary' ? colors.ink : variant === 'outline' || variant === 'ghost' ? colors.primary : colors.white} />
      ) : (
        <>
          {icon}
...
const styles = StyleSheet.create({
  base: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, borderRadius: borderRadius.lg },
  fullWidth: { width: '100%' },
