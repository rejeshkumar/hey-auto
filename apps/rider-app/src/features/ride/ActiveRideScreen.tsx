...
export function ActiveRideScreen({ navigation }: any) {
  ...
  useEffect(() => {
    socketService.on('ride:driver_assigned', (data: any) => {
      setDriverInfo({ driverId: data.driverId, driverName: data.driverName, driverPhone: data.driverPhone,
    ...
    socketService.on('ride:cancelled', (data: any) => {
      Alert.alert(t('ride.cancelRide'), data.reason || ''); resetRide(); navigation.replace('MainTabs');
    });
  ...
  const handleCancel = () => {
    Alert.alert(t('ride.cancelConfirm'), '', [
      ...
      { text: t('common.confirm'), style: 'destructive', onPress: async () => {
        try {
          if (currentRide) await rideApi.cancelRide(currentRide.id);
        } catch {}
        resetRide();
  ...
  return (
    <View style={styles.container}>
      <MapView style={styles.map}
        region={{ latitude: mapCenter.lat, longitude: mapCenter.lng, latitudeDelta: 0.01, longitudeDelta: 0.01 }}
        showsUserLocation
      >
...
const styles = StyleSheet.create({
  ...
  map: { flex: 1 },
  sosBtn: {
    position: 'absolute', top: 55, right: spacing.base,
    backgroundColor: colors.error, paddingVertical: spacing.sm, paddingHorizontal: spacing.md,
  ...
  bottomSheet: {
    ...
    padding: spacing.lg, paddingBottom: spacing.xxxl, minHeight: 200,
  },
  ...
  driverCard: {
    ...
    borderRadius: borderRadius.lg, padding: spacing.base, marginVertical: spacing.base, gap: spacing.md,
  },
  ...
  otpCard: { backgroundColor: colors.primaryLight, borderRadius: borderRadius.lg, padding: spacing.lg, alignItems: 'center', marginVertical: spacing.base },
  ...
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, paddingVertical: spacing.md, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border },
